...
<table class="cloudfw_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		<tr>
			...
			<td><?php
				$memory = cloudfw_let_to_num( WP_MEMORY_LIMIT );
		...
		<?php if ( function_exists( 'ini_get' ) ) : ?>
			...
			<tr>
				...
				<td>
					<?php if ( cloudfw_need_increase_max_input_vars() ) { ?>
						<mark class="error"><?php echo ini_get('max_input_vars'); ?> 

							(<?php printf ( __('It seems that the post variable limit imposed by your server configuration is %s. Exceeding this limit all of your page contents may not be saved. Please increase your <strong>max_input_vars</strong> directive in php.ini. %s','cloudfw') , '<strong>'. $max_input_vars . '</strong>', '<a target="_blank" href=\''. cloudfw_admin_url('global') .'#troubleshooting\'>'. __('More information','cloudfw') .'</a>' ); ?>)

						</mark>
					<?php } else { ?>
						<mark class="yes"><?php echo ini_get('max_input_vars'); ?> - <?php _e('Ok','cloudfw'); ?></mark>
					<?php } ?>
				</td>
			</tr>
			<tr>
