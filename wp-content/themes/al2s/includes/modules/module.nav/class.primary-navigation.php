    ...
    **/
    class CloudFw_Walker_Primary_Menu extends Walker_Nav_Menu {
        ...
        function display_element ($element, &$children_elements, $max_depth, $depth = 0, $args, &$output) {
            ...
            $hide_item = false;
            $condition_result = cloudfw_get_post_meta($element->ID, 'menu_logical_condition_result', 'show');
            $condition = cloudfw_get_post_meta($element->ID, 'menu_logical_condition');

            switch ($condition) {
                case 'is_logged_in':

                    if ( $condition_result == 'hide' && is_user_logged_in() ) {
                        $hide_item = true;
                    } elseif ( $condition_result == 'show' && !is_user_logged_in() ) {
                        $hide_item = true;
                    }
