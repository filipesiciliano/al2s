...
/**
 *	Adds composer contents to search
 *
 *	@since 2.0.5
 */
function cloudfw_composer_posts_join($joins) {
	...
	$joins .= " INNER JOIN {$wpdb->postmeta} ON ({$wpdb->posts}.ID = {$wpdb->postmeta}.post_id AND meta_key = 'env_composer') ";
 ...
 *	@since 2.0.5
 */
function cloudfw_composer_posts_search($search,$qry) {
	if ( !is_search() || !is_main_query() || ! apply_filters( 'cloudfw_composer_search_enable', true ) || ! cloudfw_check_onoff( 'global', 'cc_search' ) ) {
		return $search;
	}
	...
	$add = $wpdb->prepare("(CAST({$wpdb->postmeta}.meta_value AS CHAR) LIKE '%%%s%%')", $qry->get('s'));
	$pat = '|\(\((.+)\)\)|';
...
if ( !is_admin() ) {
	add_filter( 'posts_join', 'cloudfw_composer_posts_join', 1000, 2 );
	add_filter( 'posts_search', 'cloudfw_composer_posts_search', 1000, 2 );
}