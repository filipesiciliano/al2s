...
function cloudfw_breadcrumb( $args = array() ) {
	...
	if ( !empty( $trail ) && is_array( $trail ) ) {
		...
		$breadcrumb = '<' . tag_escape( $args['container'] ) . ' class="ui-bc ui-breadcrumbs breadcrumbs">';
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				...
				if ( is_paged() )
					...
					$trail[] = array( 'title' => get_the_time( __( 'd', 'cloudfw' ) ) );
			}
			...
			elseif ( get_query_var( 'w' ) ) {
				...
				if ( is_paged() )
					...
					$trail[] = array( 'title' => sprintf( __( 'Week %1$s', 'cloudfw' ), get_the_time( esc_attr__( 'W', 'cloudfw' ) ) ) );
			}
			...
			elseif ( is_month() ) {
				...
				if ( is_paged() )
					...
					$trail[] = array( 'title' => get_the_time( esc_attr__( 'F', 'cloudfw' ) ) );
			}
			...
			elseif ( is_year() ) {
				...
				if ( is_paged() )
					...
					$trail[] = array( 'title' => get_the_time( __( 'Y', 'cloudfw' ) ) );
			}
		}
